...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($fila = mysqli_fetch_assoc($resultado)) {
        ...
        if ($tipo_usuario == 'admin') {
            ...
            // Si el usuario no es un administrador, redirigir a la página de error
            header('Location: errorDeCredenciales.html');
            exit();
        }
    } else {
        // Si no se encontró un usuario, redirigir a la página de error
        header('Location: errorDeCredenciales.html');
        exit();
    }
